...
- **Determinism** — are tests deterministic? Flag: time-dependent assertions, random data without seeds, filesystem ordering assumptions, uncontrolled external dependencies. Prefer contract tests, deterministic test servers, or in-process fakes over mocks for controlling external behavior.
